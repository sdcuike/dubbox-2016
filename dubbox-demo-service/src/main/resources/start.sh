...
if [ "$1" = "debug" ]; then
    ...
    nohup java -Dspring.profiles.active=dev   $JAVA_OPTS $JAVA_MEM_OPTS    -classpath  $deployDir/config:$deployDir/lib/* com.alibaba.dubbo.container.Main > stdout.log 2>&1  &
fi
